    ...
    <div :id="uuid">
        <div class="ba-click-captcha" :class="props.class">
            ...
            <div class="captcha-refresh-box">
                ...
                <i class="fa fa-refresh captcha-refresh-btn" :title="i18n.global.t('utils.Refresh')" @click="load"></i>
        ...
        if (state.xy.length == state.captcha.text.length) {
            ...
            checkClickCaptcha(props.uuid, captchaInfo, props.unset).then((res) => {
                if (res.code == 1) {
                    ...
                    state.tip = props.error ? props.error : i18n.global.t('validate.The correct area is not clicked, please try again!')
                    setTimeout(() => {
                        load()
                    }, 1500)
                }
